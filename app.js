...
const COLLECTIONS = ["research", "fitness", "english", "ideas"];
...
function applyTheme() {
  ...
  root.style.setProperty("--primary-dark", shadeColor(themeSettings.primary, -14));
...
function render() {
  ...
  $("#researchCount").textContent = state.research.filter((item) => !item.done).length;
  ...
  renderList(
    "#todayResearchList",
    state.research.filter((item) => item.scope === "today"),
    "research",
    researchMarkup,
    { empty: "今天还没有科研待办" }
  );
  renderList(
    "#shortResearchList",
    state.research.filter((item) => item.scope === "short"),
    "research",
    researchMarkup,
    { empty: "短期项目先空着" }
  );
  renderList(
    "#longResearchList",
    state.research.filter((item) => item.scope === "long"),
    "research",
    researchMarkup,
    { empty: "长期目标先写一个也很好" }
  );

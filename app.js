...
const ACTIVE_TAB_KEY = "planner-active-tab-v1";
...
let syncTimer = null;
let activeTab = localStorage.getItem(ACTIVE_TAB_KEY) || "research";
...
function keepCurrentTab() {
  activateTab(activeTab || "research");
}
...
function bindCloudControls() {
  ...
  $("#syncBtn").addEventListener("click", async (event) => {
    event.preventDefault();
    activateTab("settings");
    await syncNow({ forceMessage: true });
  });
}
...
async function signIn() {
  if (!supabaseClient) {
    setCloudMessage("云数据库还没连上。请确认你上传的是最新版 index.html 和 app.js。");
    return;
  }
  const email = $("#authEmail").value.trim();
  const password = $("#authPassword").value;
  setSyncStatus("登录中", "pending");
...
async function signUp() {
  ...
  const email = $("#authEmail").value.trim();
  const password = $("#authPassword").value;
  setSyncStatus("注册中", "pending");
  const { data, error } = await supabaseClient.auth.signUp({ email, password });
  if (error) {
    setSyncStatus("注册失败", "error");
  ...
  updateAuthStatus();
  if (data.session) {
    setCloudMessage("注册成功，正在同步。");
    await syncNow({ forceMessage: true });
  } else {
    setCloudMessage("注册成功。若 Supabase 要求邮箱验证，请先去邮箱确认，再回来登录。");
  }
}
...
async function syncNow(options = {}) {
  if (!currentUser || !supabaseClient) {
    ...
    if (!currentUser || !supabaseClient) {
      if (options.forceMessage) setCloudMessage("还没有登录成功。请先用邮箱和密码登录，再点同步。");
      return;
    }
  }
  ...
  try {
    ...
    setSyncStatus("同步失败", "error");
    setCloudMessage(`同步异常：${error.message}`);
    return;
  }
}
